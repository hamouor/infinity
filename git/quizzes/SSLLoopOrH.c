...
size_t SlistLen(slist_node_ty *head);
slist_node_ty* OpenLoop(slist_node_ty* head);
slist_node_ty *HasLoop(slist_node_ty* head);
...
slist_node_ty *HasLoop(slist_node_ty* head)
{
    slist_node_ty* runnerA = NULL, *runnerB = NULL;

    assert(head);

    runnerA = head;
    runnerB = runnerA->next;
...
slist_node_ty* OpenLoop(slist_node_ty* head)
{
    slist_node_ty *new_head = NULL, *runnerA = NULL, *runnerB = NULL, *new_tail = NULL;
    size_t lenA = 0, lenB = 0;

    assert(head);

    runnerA = HasLoop(head);
    new_head = runnerA->next;
	runnerA->next = NULL;
	new_tail = runnerA;
	runnerB = new_head;
	runnerA = head;
...
int main()
{
	...
	if(!HasLoop(&arr[0]))
	{
		printf("HasLoop maltfunction\n");
	}

	if(SlistLen(OpenLoop(&arr[0])) != 10)
	{
		printf("OpenLoop maltfunction. returned %ld\n", SlistLen(OpenLoop(&arr[0])));
	}
